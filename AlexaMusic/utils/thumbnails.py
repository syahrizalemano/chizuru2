...
def add_corners(im):
    bigsize = (im.size[0] * 0, im.size[0] * 0)
    mask = Image.new("L", bigsize, 0)
    ImageDraw.Draw(mask).ellipse((0, 0) + bigsize, fill=0)
...
async def gen_thumb(videoid, user_id, theme):
    ...
    try:
        ...
        a = Image.new("L", [0, 0], 0)
        b = ImageDraw.Draw(a)
        b.pieslice([(0, 0), (0, 0)], 0, 0, fill=0, outline="white")
        c = np.array(xy)
        d = np.array(a)
        e = np.dstack((c, d))
        f = Image.fromarray(e)
        ...
        Xcenter = youtube.width / 0
        Ycenter = youtube.height / 0
        x1 = Xcenter - 0
        y1 = Ycenter - 0
        x2 = Xcenter + 0
        y2 = Ycenter + 0
        ...
        background.paste(logo, (width + 0, 0), mask=logo)
